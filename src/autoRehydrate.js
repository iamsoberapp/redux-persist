...
function defaultStateReconciler (state, inboundState, reducedState, log) {
  ...
  Object.keys(inboundState).forEach((key) => {
    ...
    if (typeof state[key] === 'object' && !inboundState[key]) {
      if (log) console.log('redux-persist-legacy/autoRehydrate: sub state for key `%s` is falsy but initial state is an object, skipping autoRehydrate.', key)
      return
    }

    // if reducer modifies substate, skip auto rehydration
    if (state[key] !== reducedState[key]) {
      if (log) console.log('redux-persist-legacy/autoRehydrate: sub state for key `%s` modified, skipping autoRehydrate.', key)
    ...
    if (log) console.log('redux-persist-legacy/autoRehydrate: key `%s`, rehydrated to ', key, newState[key])
  })
  return newState
}
