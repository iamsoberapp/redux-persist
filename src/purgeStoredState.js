...
export default function purgeStoredState (config, keys) {
  ...
  // basic validation
  if (Array.isArray(config)) throw new Error('redux-persist-legacy: purgeStoredState requires config as a first argument (found array). An array of keys is the optional second argument.')
  if (!storage) throw new Error('redux-persist-legacy: config.storage required in purgeStoredState')

  if (typeof keys === 'undefined') { // if keys is not defined, purge all keys
    return new Promise((resolve, reject) => {
      storage.getAllKeys((err, allKeys) => {
        if (err) {
          if (process.env.NODE_ENV !== 'production') console.warn('redux-persist-legacy: error during purgeStoredState in storage.getAllKeys')
          reject(err)
        } else {
